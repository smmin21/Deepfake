...
class FaceForensics(ImageDataset):
    ...
    def __getitem__(self, index):
        ...
        if self.transform is not None:
           frame = self.transform(frame)
        data = {'frame': frame, 'label': 0 if domain_choice==4 else 1}
        return data
...
if __name__ == "__main__":
    # import torchvision
    # traindataset = FaceForensics(root_path="/root/datasets/ff", split='train', image_size=128, transform=None, num_frames=16)
    # valdataset = FaceForensics(root_path="/root/datasets/ff", split='val', image_size=128, transform=None, num_frames=16)
    # testdataset = FaceForensics(root_path="/root/datasets/ff", split='test', image_size=128, transform=None, num_frames=16)
    
    # mean = np.zeros(3)
    # std = np.zeros(3)
    # from tqdm import tqdm
    # for samples in tqdm(traindataset+valdataset+testdataset):
    ...
    # mean /= len(traindataset)+len(valdataset)+len(testdataset)
    # std /= len(traindataset)+len(valdataset)+len(testdataset)
