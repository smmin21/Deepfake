...
def pretrained_load(enc_name, enc_args, clf_name, clf_args):
  """
  Initializes a meta model with a pre-trained encoder.
  ...
  Returns:
    model (MAML): a meta model with a pre-trained encoder.
  """
  enc = encoders.pretrained_load(enc_name, enc_args)
  clf_args['in_dim'] = enc.get_out_dim()
  clf = classifiers.make(clf_name, **clf_args)
  model = MAML(enc, clf)
  return model
...
class MAML(Module):
  ...
  def _inner_iter(self, x, y, params, mom_buffer, episode, inner_args, detach):
    ...
    with torch.enable_grad():
      ...
      loss = F.cross_entropy(logits.to(torch.float), y.to(torch.long))
      # backward pass
  ...
  def forward(self, x_shot, x_query, y_shot, inner_args={'reset_classifier':False, 'n_step':5, 'encoder_lr':0.01, 'classifier_lr':0.01, 'momentum':0, 'weight_decay':0, 'first_order':False, 'frozen':[]}, meta_train=True):
    """
    Args:
