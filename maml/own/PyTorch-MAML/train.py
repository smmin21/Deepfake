...
def main(config):
  ...
  # meta-train
  dataset_names = config['dataset'].split(', ')
  all_datasets = []
  for data_name in dataset_names:
    dataset = datasets.make(data_name, **config['train'])
    all_datasets.append(dataset)
    # utils.log('meta-train set: {} (x{}), {}'.format(
    #   train_set[data_name][0][0].shape, len(train_set[data_name]), train_set[data_name].n_classes))
  dataset_sizes = [len(dataset) for dataset in all_datasets]
  weights = 1.0 / np.array(dataset_sizes)
  weights = weights / weights.sum()
  
  sampler = WeightedRandomSampler(weights, sum(dataset_sizes), replacement=True)
  train_set = ConcatDataset(all_datasets)
  train_loader = DataLoader(
    train_set, config['train']['n_episode'],
    collate_fn=datasets.collate_fn, num_workers=1, pin_memory=True, sampler=sampler)
  ...
  if config.get('val'):
    ...
    all_val_datasets = []
    for data_name in dataset_names:
      dataset = datasets.make(data_name, **config['val'])
      all_val_datasets.append(dataset)
    val_dataset_sizes = [len(dataset) for dataset in all_val_datasets]
    val_weights = 1.0 / np.array(val_dataset_sizes)
    val_weights = val_weights / val_weights.sum()
    
    val_sampler = WeightedRandomSampler(val_weights, sum(val_dataset_sizes), replacement=True)
    val_set = ConcatDataset(all_val_datasets)
    # utils.log('meta-val set: {} (x{}), {}'.format(
    #   val_set[0][0].shape, len(val_set), val_set.n_classes))
    val_loader = DataLoader(
      val_set, config['val']['n_episode'],
      collate_fn=datasets.collate_fn, num_workers=1, pin_memory=True, sampler=val_sampler)
  ...
  if config.get('load'):
    ...
    model = models.pretrained_load(config['encoder'], config['encoder_args'],
  ...
  # torchsummary
  # torchsummary.summary(model, [(10, 3, 84, 84), (20, 3, 84, 84), (10, )], batch_size=4)
  ...
  for epoch in range(start_epoch, config['epoch'] + 1):
    ...
    # total_length = sum([len(loader) for loader in train_loader.values()])
